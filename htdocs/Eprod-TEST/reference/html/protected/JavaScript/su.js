...
function suUpdate(row_id) {
	...
	$.post("ajax/su.php",{ action: 'update_perm', rowid: row_id, status: STATUS, 
...
function suMyAdd() {
	...
	if (MY_PWD==MY_PWD2){
		if ((MY_USER != '') && (MY_PWD != '')){
			$.post("ajax/su.php",{ action: 'add_my', myuser: MY_USER, mypwd: MY_PWD },
			function(data){
...
function suPermAdd() {
	...
	if ((USER_NAME != '') && (ID_USER != '') && (NAME_EMP != '')){
		$.post("ajax/su.php",{ action: 'perm_add', status: STATUS, idbadge: ID_BADGE, username: USER_NAME, iduser: ID_USER, 
...
function susendValue(str){
	...
	$.post("ajax/su.php",{ sendValue: str, from: 'dash', df: df, dt: dt },
