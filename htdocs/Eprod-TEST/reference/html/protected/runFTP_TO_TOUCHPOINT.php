<?php
	...
	if ($retval == 0) {
		error_log("runFTP_TO_TOUCHPOINT cannot connect to database");
	} else {
		$retval = SelectDatabase($dbName);
		if ($retval == 0) {
			error_log("runFTP_TO_TOUCHPOINT cannot select " . $dbName);
		} else {
			error_log("#############################################");
			error_log("### runFTP_TO_TOUCHPOINT started at " . date('Y-m-d g:i:s a'));
			error_log("### runFTP_TO_TOUCHPOINT CHECKING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			...
			$sql .= " WHERE ";
			$sql .= "	PROC_NAME = 'runFTP_TO_TOUCHPOINT' ";
			$sql .= "	and ";
			$sql .= "	FLAG_RUNNING = '1' ";
			$sql .= "	and ";
			...
			if (mssql_num_rows($results) == 0) {
				...
				error_log("### runFTP_TO_TOUCHPOINT SETTING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
...
*/
				...
				error_log("### runFTP_TO_TOUCHPOINT DELETING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
				error_log("### LAST INSERT ID: " . $ProcRowID);
				...
				error_log("### runFTP_TO_TOUCHPOINT ALREADY RUNNING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			}	
		}		
		error_log("### runFTP_TO_TOUCHPOINT finished at " . date('Y-m-d g:i:s a'));
		error_log("#############################################");

		$retval = DisconnectFromDatabaseServer($db);
		if ($retval == 0) {
			error_log("runFTP_TO_TOUCHPOINT cannot disconnect from database");
		}
	}
?>