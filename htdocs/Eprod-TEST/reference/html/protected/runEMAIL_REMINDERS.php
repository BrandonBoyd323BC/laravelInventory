<?php
	...
	if ($retval == 0) {
		error_log("runEMAIL_REMINDERS cannot connect to database");
	} else {
		$retval = SelectDatabase($dbName);
		if ($retval == 0) {
			error_log("runEMAIL_REMINDERS cannot select " . $dbName);
		} else {
			error_log("#############################################");
			error_log("### runEMAIL_REMINDERS started at " . date('Y-m-d g:i:s a'));
			error_log("### runEMAIL_REMINDERS CHECKING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			...
			$sql .= " WHERE ";
			$sql .= "	PROC_NAME = 'runEMAIL_REMINDERS' ";
			$sql .= "	and ";
			$sql .= "	FLAG_RUNNING = '1' ";
			$sql .= "	and ";
			...
			if (mssql_num_rows($results) == 0) {
				...
				error_log("### runEMAIL_REMINDERS SETTING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
				...
				QueryDatabase($sql, $results);
/*
				error_log("### runEMAIL_REMINDERS CHECKING CP_SHPLIN ");
				...
				while ($row = mssql_fetch_assoc($results)) {
					error_log("### runEMAIL_REMINDERS found ZERO DOLLAR ID_ORD: " . $row['id_ord']);
					...
					if (mssql_num_rows($results0) == 0) {
						error_log("### runEMAIL_REMINDERS INSERTING RECORD for ID_ORD: " . $row['id_ord']);
						...
						error_log("### runEMAIL_REMINDERS Alert for ID_ORD: " . $row['id_ord'] . " ID_SHIP: " . $row['id_ship'] . " SEQ_LINE_ORD: " . $row['seq_line_ord'] . " Previously sent.");
					}
				}
*/


				$subject = "This is a test WITH HTML";
				$body = "Greetings," .
					"\r\n\r\nThis message is a reminder to COMPANY XYZ that your account with National Safety Apparel " .
					"\r\nhas an overdue balance of $2,500." .
					"\r\nPlease pay National Safety Apparel." .
					"\r\nThank you";
				...
				<html>
				<head>
				<title>National Safety Apparel</title>
				</head>
				<body>
				<p>Greetings:</p>
				<p>Your account with National Safety Apparel currently holds an outstanding balance of $2,500.  Please settle this debt.</p>
				<table>
				<tr>
				<th>Firstname</th>
				<th>Lastname</th>
				...
				<img src='images\NSA_logo.gif' alt='logo'>
				</body>
				</html>
				";
				...
				error_log("### runEMAIL_REMINDERS DELETING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
				error_log("### LAST INSERT ID: " . $ProcRowID);
				...
				error_log("### runEMAIL_REMINDERS ALREADY RUNNING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			}

			error_log("### runEMAIL_REMINDERS finished at " . date('Y-m-d g:i:s a'));
			error_log("#############################################");
		}
		$retval = DisconnectFromDatabaseServer($db);
		if ($retval == 0) {
			error_log("runEMAIL_REMINDERS cannot disconnect from database");
		}
	}
?>
