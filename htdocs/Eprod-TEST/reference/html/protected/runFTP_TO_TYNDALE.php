<?php
	...
	if ($retval == 0) {
		error_log("runFTP_TO_TYNDALE cannot connect to database");
	} else {
		$retval = SelectDatabase($dbName);
		if ($retval == 0) {
			error_log("runFTP_TO_TYNDALE cannot select " . $dbName);
		} else {
			error_log("#############################################");
			error_log("### runFTP_TO_TYNDALE started at " . date('Y-m-d g:i:s a'));
			error_log("### runFTP_TO_TYNDALE CHECKING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			...
			$sql .= " WHERE ";
			$sql .= "	PROC_NAME = 'runFTP_TO_TYNDALE' ";
			$sql .= "	and ";
			$sql .= "	FLAG_RUNNING = '1' ";
			$sql .= "	and ";
			...
			if (mssql_num_rows($results) == 0) {
				...
				$sql .= " DATE_EXP ";
				$sql .= ") VALUES ( ";
				$sql .= "'runFTP_TO_TYNDALE', ";
				$sql .= "1, ";
				$sql .= " getDate(), ";
				$sql .= " dateadd(minute,20,getDate()) ";
				$sql .= ")  SELECT LAST_INSERT_ID=@@IDENTITY";
				QueryDatabase($sql, $results);
				$row = mssql_fetch_assoc($results);
				$ProcRowID = $row['LAST_INSERT_ID'];
				error_log("### runFTP_TO_TYNDALE SETTING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
				...
				//	$baseDir = '/mnt/TyndaleFTP_TEST/Outbound';
				//	$SqlTableName_FTP_FILES = "nsa.TYNDALE_FTP_FILES_TEST";
				//} else {
					$ftp_server 	= "ftp.Tyndalelogistics.com";  	//LIVE Tyndale
					$ftp_user_name 	= "nsa@Tyndalelogistics.com";
					$ftp_user_pass 	= "nsa$2016";
					$baseDir = '/mnt/TyndaleFTP/Outbound';
					$SqlTableName_FTP_FILES = "nsa.TYNDALE_FTP_FILES";
				//}
					...
					if ((!$conn_id) || (!$login_result)) {
						...
						foreach ($PendingFiles as $File) {
							...
							if (!$upload) {
								...
								$subject = "FTP to Tyndale failed " . $File;
								$body = "FTP to Tyndale failed " . $File . "\r\nTransmission will retry on next script execution.";
...
*/
				...
				if ($numFiles > 0) {
					...
					foreach ($PendingFiles as $File) {
						// Connect to FTP Server
						//$conn_id = ftp_connect($ftp_server,36879,30);
						$resConnection = ssh2_connect($ftp_server, 36879);
						// Login to FTP Server
						//$login_result = ftp_login($conn_id, $ftp_user_name, $ftp_user_pass);
						$login_result = ssh2_auth_password($resConnection, $strServerUsername, $strServerPassword);
						// Verify Log In Status
						//if ((!$conn_id) || (!$login_result)) {
						if ((!$resConnection) || (!$login_result)) {
							error_log("### FTP connection has failed!");
							error_log("### Attempted to connect to " . $ftp_server . " for user " . $ftp_user_name);
						} else {
							error_log("### Connected to " . $ftp_server . ", for user " . $ftp_user_name);
...
*/							
				...
				error_log("### runFTP_TO_TYNDALE DELETING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
				error_log("### LAST INSERT ID: " . $ProcRowID);
				...
				error_log("### runFTP_TO_TYNDALE ALREADY RUNNING flag in RUNNING_PROC " . date('Y-m-d g:i:s a'));
			}	
		}		
		error_log("### runFTP_TO_TYNDALE finished at " . date('Y-m-d g:i:s a'));
		error_log("#############################################");

		$retval = DisconnectFromDatabaseServer($db);
		if ($retval == 0) {
			error_log("runFTP_TO_TYNDALE cannot disconnect from database");
		}
	}
?>