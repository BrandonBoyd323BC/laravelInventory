	...
	if ($retval == 0) {
		...
		if ($retval == 0) {
			...
			if (isset($_POST["sendAddValue"]) && isset($_POST["so1"]) && isset($_POST["sufx_so1"]) && isset($_POST["num_recs"])){
				...
				//SEARCH FOR EXISTING RECORD FOR SHOP ORDER
				$sql  = "SELECT * FROM nsa.SHPORD_LABELS_READY ";
				$sql .= " WHERE ID_SO = '".$SO1."' ";
				...
				if (mssql_num_rows($results) > 0) {
					while ($row = mssql_fetch_assoc($results)) {
						...
						$sql0  = "UPDATE nsa.SHPORD_LABELS_READY SET ";
					...
					//INSERT SO RECORD
					$sql0  = "INSERT INTO nsa.SHPORD_LABELS_READY (";
					$sql0 .= " ID_SO, ";
					$sql0 .= " SUFX_SO, ";
...
function refreshMarkerNumRecs($NUM_RECS,$SEARCH_SO='') {
	...
	$sql .= " sa.rowid ";
	$sql .= " from nsa.SHPORD_LABELS_READY sa ";
	if ($SEARCH_SO <> 'ALL') {
		$sql .= " 	WHERE sa.ID_SO like '" . $SEARCH_SO . "%' ";
	}

	$sql .= " order by sa.DATETIME_APPROVED desc ";
	QueryDatabase($sql, $results);
