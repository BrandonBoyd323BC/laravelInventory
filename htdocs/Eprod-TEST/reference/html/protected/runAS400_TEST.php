<?php
	...
	error_log("#############################################");
	error_log("### runAS400_TEST started at " . date('Y-m-d g:i:s a'));
	$retvalODBC = odbc_ConnectToDatabaseServer($odbc_DSN);
	if ($retvalODBC == 0) {
		error_log("### runAS400_TEST cannot connect to " . $odbc_DSN);
	} else {
		error_log("### runAS400_TEST CONNECTED to " . $odbc_DSN);
		$retval = ConnectToDatabaseServer($DBServer, $db);
		if ($retval == 0) {
			error_log("runAS400_TEST cannot connect to " . $db);
		} else {
			$retval = SelectDatabase($dbName);
			if ($retval == 0) {
				error_log("runAS400_TEST cannot select " . $dbName);
			} else {
				error_log("### runAS400_TEST querying TRACKING");
				...
				if ($odbc_num_rows > 0) {
					...
					error_log("### runAS400_TEST CHECKING FOR AS400_TRACKING_TEMP ");
					$sql = " IF OBJECT_ID('nsa.AS400_TRACKING_TEMP', 'U') IS NOT NULL";
					$sql .= "	DROP TABLE nsa.AS400_TRACKING_TEMP";
					QueryDatabase($sql, $results);

					error_log("### runAS400_TEST CREATING AS400_TRACKING_TEMP ");
					...
					$rownum = 0;
					error_log("### runAS400_TEST POPULATING nsa.AS400_TRACKING_TEMP with " . $odbc_num_rows . " records");
					while ($as400_row = odbc_fetch_array($as400_results)) {
						if (++$rownum % 50 == 0) {
							error_log("### runAS400_TEST row " . $rownum . " of " . $odbc_num_rows . " records");
						}
					...
					error_log("### runAS400_TEST CHECKING FOR nsa.AS400_TRACKING ");
					$sql = " IF OBJECT_ID('nsa.AS400_TRACKING', 'U') IS NOT NULL";
					$sql .= "	DROP TABLE nsa.AS400_TRACKING";
					QueryDatabase($sql, $results);

					error_log("### runAS400_TEST RENAMING nsa.AS400_TRACKING_TEMP to nsa.AS400_TRACKING ");
		...
		if ($retval == 0) {
			error_log("runAS400_TEST cannot disconnect from " . $db);
		}

		odbc_close($odbc_db);
		error_log("### runAS400_TEST DISCONNECTED from AS400");
	}
	error_log("### runAS400_TEST finished at " . date('Y-m-d g:i:s a'));
	error_log("#############################################");
