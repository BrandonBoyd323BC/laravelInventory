<?php
	...
	error_log("#############################################");
	error_log("### runAS400_INVOICE started at " . date('Y-m-d g:i:s a'));
	$retvalODBC = odbc_ConnectToDatabaseServer($odbc_DSN);
	if ($retvalODBC == 0) {
		error_log("### runAS400_INVOICE cannot connect to " . $odbc_DSN);
	} else {
		error_log("### runAS400_INVOICE CONNECTED to " . $odbc_DSN);
		$retval = ConnectToDatabaseServer($DBServer, $db);
		if ($retval == 0) {
			error_log("runAS400_INVOICE cannot connect to " . $db);
		} else {
			$retval = SelectDatabase($dbName);
			if ($retval == 0) {
				error_log("runAS400_INVOICE cannot select " . $dbName);
			} else {
				error_log("### runAS400_INVOICE querying TRACKING");
				...
				if ($odbc_num_rows > 0) {
					...
					error_log("### runAS400_INVOICE CHECKING FOR AS400_INVOICE_TEMP ");
					$sql = " IF OBJECT_ID('nsa.AS400_INVOICE_TEMP', 'U') IS NOT NULL";
					$sql .= "	DROP TABLE nsa.AS400_INVOICE_TEMP";
					QueryDatabase($sql, $results);

					error_log("### runAS400_INVOICE CREATING AS400_INVOICE_TEMP ");
					$sql  = "CREATE TABLE [nsa].[AS400_INVOICE_TEMP]( ";
					$sql .= " [MDORD#] [numeric](8,0) NOT NULL, "; 
					$sql .= " [CTACCT] [varchar](10) NOT NULL, ";
					$sql .= " [CTSHP] [varchar](5) NOT NULL, "; 
					...
					$rownum = 0;
					error_log("### runAS400_INVOICE POPULATING nsa.AS400_INVOICE_TEMP with " . $odbc_num_rows . " records");
					while ($as400_row = odbc_fetch_array($as400_results)) {
						if (++$rownum % 50 == 0) {
							error_log("### runAS400_INVOICE row " . $rownum . " of " . $odbc_num_rows . " records");
						}

						//error_log($as400_row['XORD'] . " " . $as400_row['XNAME']);
						$sql2  = "INSERT INTO nsa.AS400_INVOICE_TEMP( ";
						$sql2 .= " MDORD#, "; 
						$sql2 .= " CTACCT, ";
						$sql2 .= " CTSHP, "; 
						$sql2 .= " CTCTN#, "; 
					...
					error_log("### runAS400_INVOICE CHECKING FOR nsa.AS400_INVOICE ");
					$sql = " IF OBJECT_ID('nsa.AS400_INVOICE', 'U') IS NOT NULL";
					$sql .= "	DROP TABLE nsa.AS400_INVOICE";
					QueryDatabase($sql, $results);

					error_log("### runAS400_INVOICE RENAMING nsa.AS400_INVOICE_TEMP to nsa.AS400_INVOICE ");
					$sql = " SP_RENAME 'nsa.AS400_INVOICE_TEMP','AS400_INVOICE'";
					QueryDatabase($sql, $results);
		...
		if ($retval == 0) {
			error_log("runAS400_INVOICE cannot disconnect from " . $db);
		}

		odbc_close($odbc_db);
		error_log("### runAS400_INVOICE DISCONNECTED from AS400");
	}
	error_log("### runAS400_INVOICE finished at " . date('Y-m-d g:i:s a'));
	error_log("#############################################");
