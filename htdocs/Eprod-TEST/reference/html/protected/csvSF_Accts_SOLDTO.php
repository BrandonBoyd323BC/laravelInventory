<?php
	...
	if ($retval == 0) {
		...
		if ($retval == 0) {
			error_log("csvSF_Accts_SOLDTO cannot select " $dbName);
		} else {
			$filename = "NSA_SF_SOLDTO_". date('Ymd-His') .".csv";
			error_log("#############################################");
			error_log("### csvSF_Accts_SOLDTO started at " . date('Y-m-d g:i:s a'));
			...
			//$sql .= "	-- "" as Parent, ";
			$sql .= "		PHONE as Phone, ";
			$sql .= "		ADDR_CUST_1 as AddressSold1, ";
			$sql .= "		ADDR_CUST_2 as AddressSold2, ";
			$sql .= "		ADDR_CUST_3 as AddressSold3, ";
			$sql .= "		ADDR_CUST_4 as AddressSold4, ";
			...
			$sql .= "		PROV, ";
			$sql .= "		COUNTRY, ";
			//$sql .= "	--	NAME_CONTACT_CUST as CustomerContact, ";
...
*/
			...
			fclose($fp);
			error_log("### csvSF_Accts_SOLDTO finished at " . date('Y-m-d g:i:s a'));
			error_log("#############################################");
		}
		$retval = DisconnectFromDatabaseServer($db);
		if ($retval == 0) {
			error_log("csvSF_Accts_SOLDTO cannot disconnect from database");
		}
	}
